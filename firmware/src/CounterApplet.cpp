...
void CounterApplet::handleClick() {
    // 1. Increment counter immediately (pure decimal arithmetic, no integer overflow)
    ...
    if (lifetime >= 100) unlocks |= 0x01;
    if (lifetime >= 500) unlocks |= 0x02;
    if (lifetime >= 5000) unlocks |= 0x04;
    if (lifetime >= 10000) unlocks |= 0x08;
    if (lifetime >= 75000) unlocks |= 0x10;
    if (lifetime >= 30000) unlocks |= 0x20;
    if (lifetime >= 175000) unlocks |= 0x40;
    ...
    boulderRotPhase = static_cast<uint8_t>((boulderRotPhase + 1) % BOULDER_FRAMES);
...
void CounterApplet::draw() {
    ...
    if (climbProgress <= 18) {
        // Starts on flat ground plane at base (charX=18, bldX=38), climbs onto the hill
    ...
    if (animMode == SisyphusAnimMode::IDLE_WALKING) {
        // Resting posture: braced with one hand holding the rock so it doesn't fall down!
        ...
        sisBmp = (const uint8_t*)pgm_read_ptr(&(sisyphus_push_frames[pushFrame % 4]));
        sisW = SISYPHUS_PUSH_WIDTH;
        sisH = SISYPHUS_PUSH_HEIGHT;
    }

    // Render scene with bottom-right corner hill, downward moving flat ground plane, and resting brace
